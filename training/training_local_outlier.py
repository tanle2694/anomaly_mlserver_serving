...
# 1. BUILD PIPELINE
# =========================
def build_pipeline(contamination="auto", n_neighbors: int = 20):
    return Pipeline([
...
def parse_args():
    ...
    parser.add_argument(
        "--contamination",
        default="auto",
        help="LocalOutlierFactor contamination: 'auto' or float 0-0.5 (default: auto)",
    )
    parser.add_argument(
        "--n-neighbors",
        type=int,
        default=20,
        help="Number of neighbors for LocalOutlierFactor (default: 20)",
    )
...
def main():
    ...
    print("Building Local Outlier Factor pipeline...")
    pipeline = build_pipeline(contamination, args.n_neighbors)
